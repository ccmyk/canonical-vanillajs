...
export function checkEl(entry) {
  if (entry.isIntersecting == true) {
    this.meshes[parseInt(entry.target.dataset.pg)].y =
      this.meshes[parseInt(entry.target.dataset.pg)].el.querySelector('.cv').getBoundingClientRect()
        .y + window.scrollY;
...
export function updateX(x = 0, anim = null) {
  ...
  if (anim == null) {
    for (let [i, a] of this.meshes.entries()) {
      ...
      a.mesh.position.x =
        -(this.viewport[0] / 2) + a.mesh.scale.x / 2 + (x / this.screen[0]) * this.viewport[0];
    }
  } else {
  ...
  if (all != null) {
    ...
    for (let [i, a] of this.mmap.entries()) {
      if (this.meshes[a].vid) {
        if (
          this.meshes[a].texture.image.readyState >= this.meshes[a].texture.image.HAVE_ENOUGH_DATA
        ) {
          if (!this.meshes[a].texture.image)
            this.meshes[a].texture.image = this.meshes[a].texture.image;
      ...
      if (this.onanim != -1) {
        let ypos = (this.meshes[a].y =
          this.meshes[a].el.querySelector('.Oi').getBoundingClientRect().y + y);
      }
      let ypos = this.meshes[a].y;
...
export function updateScale(anim = null) {
  ...
  if (anim == null) {
    ...
    for (let [i, a] of this.mmap.entries()) {
      this.meshes[a].mesh.scale.x =
        (this.viewport[0] * (this.bound[2] * this.meshes[a].multx)) / this.screen[0];
      this.meshes[a].mesh.scale.y =
        (this.viewport[1] * (this.bound[3] * this.meshes[a].multy)) / this.screen[1];
